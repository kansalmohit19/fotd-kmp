...
class UserApiImpl(
    ...
    override suspend fun dailyFact(
        url: String,
        accessToken: String,
    ): HttpResponse =
        httpClient.get("http://152.67.10.2:8080/fact/today") {
    ...
    override suspend fun validateToken(request: LoginUserRequest): HttpResponse =
        httpClient.post("http://152.67.10.2:8080/user/loginViaAccessToken") {
            setBody(FormDataContent(request.toParameters()))
        }

    override suspend fun factsList(
        url: String,
        accessToken: String,
    ): HttpResponse =
        httpClient.get(url) {
